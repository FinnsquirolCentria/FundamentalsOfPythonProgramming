...
class Reservation:
    """Represents a reservation with attributes and useful methods"""
    ...
    def is_long(self) -> bool:
        """Check if reservation is long (≥ 3 hours)"""
...
def convert_reservation_data(data: list) -> Reservation:
    """
    Convert data types to meet program requirements

    Parameters:
     data (list): Unconverted reservation -> 11 columns 

    Returns:
     Reservation: Converted data as Reservation object
    """
    return Reservation(
        ...
        resource=str(data[9]),
        created=datetime.strptime(str(data[10]).strip(), "%Y-%m-%d %H:%M:%S"),
    )
...
def fetch_reservations(reservation_file: str) -> list[Reservation]:
    ...
    Returns:
     reservations (list): Read and converted reservations as Reservation objects
    """
    reservations = []
...
def total_revenue(reservations: list) -> None:
    ...
    Parameters:
     reservations (list): Reservations
    """
    revenue : float = sum(x.total_price() for x in reservations if x.confirmed)
